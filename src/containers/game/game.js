...
class Game extends Component {
  ...
  componentDidMount() {
    ...
    const socket = io(
      "http://localhost:3001/?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MywiaWF0IjoxNTI4MzQzMDExfQ.Vr6rFksDbL6HZw8FMg5N6O7RhPD1un8S_ZN6WEao9Ac"
    );
        ...
        {possibleAnswers && (
          <div>
            {possibleAnswers.map(answer => {
              return (
                <button key={answer} onClick={() => this.submitAnswer(answer)}>
                  {answer}
                </button>
              );
            })}
