...
export function requestPasswordReset(email) {
  const url = `/user/passwordreset?email=${email}`;
  const config = {
    url,
    method: 'get',
    header: {
  ...
  return callApi(config, requestPasswordResetRequest, requestPasswordResetSuccess, requestPasswordResetFailure);
}
...
export function submitPasswordReset(new_password, reset_token) {
  ...
  return callApi(config, submitPasswordResetRequest, submitPasswordResetSuccess, submitPasswordResetFailure);
}
